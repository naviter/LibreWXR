...
- **Lightning:** Point-strike layer (not a grid) served at `/v2/lightning?since=<sec>&bbox=<min_lon,min_lat,max_lon,max_lat>` as JSON flash records `{t, lat, lon, energy, age}`; the example viewers render one cross-hair per flash, faded by age. Two networks merged at query time: **GLM** (NOAA GOES-GLM total lightning over the Americas, CC0 public domain, anonymous S3 `noaa-goes19`/`noaa-goes18`, LCFA NetCDF granules ~20 s — always on) and **MTG-LI** (EUMETSAT Meteosat Third Generation Lightning Imager over Europe/Africa/S.America, free with attribution, EUMDAC OAuth + Data Store ZIP→BODY NetCDF — dormant until `LIBREWXR_EUMETSAT_KEY`/`_SECRET` set). Sources live under `sources/lightning/{glm,mtg_li}/` and share `sources/lightning/_common.py`'s in-memory `FlashStore` (rolling `lightning_retention_minutes` window, no memmap). Wired through the same discovery walker as the other families via a `lightning_provider(settings, cache_dir)` returning a `LightningContribution`; `LIBREWXR_LIGHTNING_ENABLED=false` returns 503. GLM flashes are stamped at granule scan-start (the per-flash time-offset var is sub-20 s packing, not absolute epoch); MTG-LI `flash_time` is seconds since 2000-01-01T00:00Z.
